...
export function createApp() {
  ...
  // Internal ingest (relay -> here). Public brand APIs under /api/v1.
  app.use("/", events);
  app.use("/api/v1", webhooks, engagements);
...
if (process.env.NODE_ENV !== "test") {
  ...
  createApp().listen(port, () => console.log(`events-service listening on ${port}`));
}
