...
**Purpose.** Usage tracking, plans and subscriptions, payment processor hooks.

**The rule that matters most here.** v1 tracks usage and subscription state only. Invoicing is deferred - do not build it without a decision.

## Ownership
- Owns: `plans, subscriptions, usage_metrics`
- Database: `xappx_billing_service` — its own. No other service connects to it.
- External identifiers held as plain uuids with no foreign keys: `app_id, client_id, user_id`
- Publishes: com.xappx.subscription.changed
...
- Contract test against `contracts/openapi/billing-service.v1.yaml`.
