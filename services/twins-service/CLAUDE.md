...
**Purpose.** Twin creation, training sources, model parameters.

**The rule that matters most here.** Gated by the twins product toggle, checked at this service boundary - not in the UI and not only at the gateway.

## Ownership
- Owns: `twins, twin_training_sources`
- Database: `xappx_twins_service` — its own. No other service connects to it.
- External identifiers held as plain uuids with no foreign keys: `app_id, client_id, owner_id, file_id, avatar_id`
- Publishes: com.xappx.twin.created, com.xappx.twin.ready
- Consumes: com.xappx.application.product.disabled, com.xappx.vault.item.stored
...
- Contract test against `contracts/openapi/twins-service.v1.yaml`.
