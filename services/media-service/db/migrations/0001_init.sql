-- media-service · owns: media_assets, provider_jobs
--
-- Separate database. Cross-service identifiers (app_id) are plain uuid columns
...
create table media_assets (
  ...
  kind         text not null check (kind in ('image','video','audio','avatar')),
  provider     text,
  provider_ref text,
  storage_key  text,
...
create policy media_tenant on media_assets using (app_id = current_app_id());
