...
**Purpose.** Assembles the export package by calling every other service, and validates it.

**The rule that matters most here.** The package must contain full ledger history, not balances. Validation compares row counts and checksums before the package is released.

## Ownership
- Owns: `export_jobs, export_artifacts`
- Database: `xappx_export_service` — its own. No other service connects to it.
- External identifiers held as plain uuids with no foreign keys: `app_id, client_id`
- Publishes: com.xappx.export.completed
- Consumes: nothing
...
- Contract test against `contracts/openapi/export-service.v1.yaml`.
