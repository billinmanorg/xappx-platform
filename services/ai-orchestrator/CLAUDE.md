...
**Purpose.** Capability routing across AI providers, with cost and latency tracking.

**The rule that matters most here.** Callers request a capability, never a provider. Routing is data in ai_routes, changeable without a deploy.

## Ownership
- Owns: `ai_routes, ai_requests`
- Database: `xappx_ai_orchestrator` — its own. No other service connects to it.
- External identifiers held as plain uuids with no foreign keys: `app_id, user_id`
- Publishes: com.xappx.ai.request.completed
- Consumes: nothing
...
- Contract test against `contracts/openapi/ai-orchestrator.v1.yaml`.
