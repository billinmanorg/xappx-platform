...
**Purpose.** Append-only audit trail of admin actions, config changes and consent.

**The rule that matters most here.** Append only. No updates, no deletes, ever - enforced by trigger, not convention.

## Ownership
- Owns: `audit_log, consents`
- Database: `xappx_audit_service` — its own. No other service connects to it.
- External identifiers held as plain uuids with no foreign keys: `app_id, client_id, actor_id`
- Publishes: nothing
...
- Contract test against `contracts/openapi/audit-service.v1.yaml`.
